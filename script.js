...
document.getElementById('submit-quiz-btn').addEventListener('click', function() {
  ...
  localStorage.setItem(uniqueId, JSON.stringify({ quizData: quizData, showAnswers: showAnswers }));
  // توليد رابط الاختبار
  var quizLink = window.location.origin + window.location.pathname.replace("index.html", "") + "quiz.html?quizid=" + uniqueId;
